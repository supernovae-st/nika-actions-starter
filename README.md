...
| File | What it does |
|---|---|
| `flows/pr-risk-review.nika` | the signature flow: read a PR diff, judge its risk, comment only when it is high. Declares its tightest permits (`exec: ["git"]` and one MCP tool, everything else default-deny) |
